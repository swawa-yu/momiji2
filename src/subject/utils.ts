 ...
 * @returns Kaisetsuki
 */
export function parseKaisetsuki(
  shouldBeKaisetsuki: string
): Kaisetsuki | undefined {
  ...
  // };
  try {
    if (!semesters.some((v) => v === splitted[1]))
      throw new Error('解析エラー');
    if (!jikiKubuns.some((v) => v === splitted[2]))
      throw new Error('解析エラー');
    return {
      rishuNenji: parseInt(splitted[0][0]),
