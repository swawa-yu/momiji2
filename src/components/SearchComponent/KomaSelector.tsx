...
const KomaSelector: React.FC<KomaSelectorProps> = ({
  onSelectionChange: onScheduleChange,
}) => {
  // 初期状態で全ての曜日とコマをtrueに設定
  const [youbiKoma, setYoubiKoma] = useState<YoubiKomaSelected>(
    initializeYoubiKoma(true)
  );
  ...
  return (
    <div className="koma-selector">
      <Button size="small" variant="text" onClick={() => setAllYoubiKoma(true)}>
        全て選択
      </Button>
      <Button
        size="small"
        variant="text"
        onClick={() => setAllYoubiKoma(false)}
      >
        全て解除
      </Button>
      <table>
              ...
              <th key={youbi}>
                <Button
                  ...
                  onClick={() =>
                    setAllYoubi(
                      youbi,
                      !komas.every((koma) => youbiKoma[`${youbi}${koma}`])
                    )
                  }
                  sx={{
                    padding: '4px 8px',
            ...
            <tr key={koma}>
              ...
              <td>
                <Button
                  ...
                  onClick={() =>
                    setAllKoma(
                      koma,
                      !youbis.every((youbi) => youbiKoma[`${youbi}${koma}`])
                    )
                  }
                  sx={{
                    padding: '4px 8px',
              ...
              {youbis.map((youbi) => (
                ...
                <td
                  key={youbi}
                  onClick={() =>
                    handleYoubiKomaCheckboxChange(
                      `${youbi}${koma}` as YoubiKoma,
                      !youbiKoma[`${youbi}${koma}`]
                    )
                  }
                  style={{
                    cursor: 'pointer',
      ...
      <FormControlLabel
        control={
          <Checkbox
            ...
            onChange={(e) =>
              handleYoubiKomaCheckboxChange('集中', e.target.checked)
            }
          />
        }
        label="集中"
