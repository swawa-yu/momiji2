...
1. [#19 シラバスデータの更新を自動で行う](https://github.com/swawa-yu/momiji2/issues/19)
   - まず`pnpm import:harvester-generation --manifest <Harvester manifest> --departments <artifact> --check`で検証する。reviewが報告された場合だけ内容確認後、`pnpm import:harvester-generation --manifest <Harvester manifest> --departments <artifact> --accept-review`（reviewなしはflag不要）で取り込み、`pnpm generate:subject-constants`を実行する。
