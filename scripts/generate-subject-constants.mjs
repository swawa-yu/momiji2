...
function renderActiveSubjectData(manifest) {
  const dataPath = JSON.stringify(`../../data/${manifest.dataFile}`);
  const rawPropertiesPath = JSON.stringify('../types/rawSubjectProperties');
  return `// Generated from data/subjectDataManifest.json
// Do not edit this file directly.

import { rawSubjectProperties } from ${rawPropertiesPath};
import type { RawSubjectMap } from '../types/subject';

export const subjectDataManifest = ${JSON.stringify(manifest, null, 2)} as const;

export const subjectDataUrl = new URL(${dataPath}, import.meta.url).href;
...
export function validateRawSubjectMap(value: unknown): asserts value is RawSubjectMap {
  if (!isPlainObject(value)) {
    throw new Error('Subject data must be a plain object.');
  }
  if (Object.keys(value).length !== subjectDataManifest.subjectCount) {
    throw new Error(
      'Subject data count does not match the active manifest.'
    );
  }
  ...
  for (const [subjectCode, subject] of Object.entries(value)) {
    if (!isPlainObject(subject)) {
      throw new Error(
        'Subject ' + subjectCode + ' must be a plain object.'
      );
    }
    ...
    if (unexpectedProperty || subjectKeys.length !== rawSubjectProperties.length) {
      throw new Error(
        'Subject ' + subjectCode + ' has an unexpected field.'
      );
    }
    ...
    if (subject['年度'] !== subjectDataManifest.academicYear) {
      throw new Error(
        'Subject ' + subjectCode + ' has a mismatched 年度.'
      );
    }
  }
}
...
export async function loadSubjectData(
  ...
  try {
    ...
    throw new Error(
      'Subject data response was not valid JSON.',
      { cause: error }
    );
  }
